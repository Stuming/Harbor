...
path_project='/nfs/t3/workingshop/baihaohao/test_gump_pr2'
path_rawdata='/nfs/s1/data/gumpdata/ds000113d_R2.0.0'
fmri_suffix='ses-movie/func'

# TODO 
for i in [10]: #range(1,21):

    subid_str=str(i)
    while len(subid_str)<2:
        subid_str='0'+subid_str
    subid='sub-'+subid_str  # build sub-xxx, stands for anatomy.

    sessid=str(i)
    while len(sessid)<3:
        sessid='0'+sessid
    sessdir='S'+sessid  # build Sxxx, stands for function.
